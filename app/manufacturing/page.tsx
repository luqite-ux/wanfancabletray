...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return buildPageMetadata({ title: pageTitle, description: pageDescription, path: "/manufacturing", locale, supportedLocales });
}
...
export default async function ManufacturingPage() {
  ...
  return (
    <main>
      ...
      <section className="content-section manufacturing-evidence"><div className="page-container manufacturing-evidence__grid"><div className="manufacturing-evidence__image"><Image alt="Wide view of the Wanfan manufacturing workshop" fill sizes="(max-width: 860px) 100vw, 48vw" src="/assets/factory/workshop-01.jpg" style={{ objectFit: "cover" }} /></div><div><p className="eyebrow">Verified production context</p><h2>{productionFacts.facilityArea.display} facility and {productionFacts.machineCount.display} machines.</h2><p>The typical production window is {productionFacts.productionWindow.display}. Scheduling depends on confirmed configuration, quantity, and current production planning.</p><dl className="fact-list"><div><dt>Facility area</dt><dd>{productionFacts.facilityArea.display}</dd></div><div><dt>Production equipment</dt><dd>{productionFacts.machineCount.display} machines</dd></div><div><dt>Typical window</dt><dd>{productionFacts.productionWindow.display}</dd></div></dl></div></div></section>
